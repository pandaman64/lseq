...
fn pick_index<R: Rng + ?Sized>(rng: &mut R, left: usize, right: usize, strategy: InsertionStrategy) -> usize {
...
fn get_strategy<R: Rng + ?Sized>(rng: &mut R, strategies: &mut Vec<InsertionStrategy>, level: usize) -> InsertionStrategy {
...
impl<SiteId: Ord + Clone> Key<SiteId> {
    fn pick(&self, other: &Self, site_id: Id<SiteId>, strategies: &mut Vec<InsertionStrategy>) -> Self {
        assert!(*self < *other);
...
impl<SiteId: Ord + Clone + std::fmt::Debug, Value> Document<SiteId, Value> {
    ...
    pub fn insert(&mut self, site_id: SiteId, left: &Key<SiteId>, right: &Key<SiteId>, value: Value) -> Key<SiteId> {
        let key = left.pick(right, Id::Site(site_id), &mut self.strategies);
        assert!(left < &key && &key < right, "must hold {:?} < {:?} < {:?}", left, key, right);
        assert!(self.content.insert(key.clone(), Some(value)).is_none(), "key collided: {:?}", key);
        key
    }
